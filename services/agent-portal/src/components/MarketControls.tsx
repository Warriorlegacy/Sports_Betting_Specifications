    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 p-4 rounded-xl bg-slate-900/60 border border-slate-800">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-base font-bold text-slate-100">Live Market Control & Emergency Killswitch</h2>
            <p className="text-xs text-slate-400">Freeze order matching or settle positions instantly</p>
          </div>
        </div>
        ...
        {isAdmin && (
          <button
            ...
            className="flex items-center space-x-2 px-4 py-2 text-sm font-semibold rounded-xl bg-blue-600 hover:bg-blue-500 text-white shadow-lg shadow-blue-600/20"
          >
        ...
        {markets.map((m) => (
          ...
          >
            <div>
              ...
              {/* Event Name */}
              <h3 className="font-bold text-base text-slate-100 tracking-tight leading-snug mb-1">
                {m.eventName}
              </h3>
              <p className="text-xs text-slate-500 mono-num mb-4">ID: {m.id}</p>
              ...
              <div className="flex items-center space-x-2 pt-3 border-t border-slate-800">
                ...
                {isAdmin && (
                  <button
                    ...
                    className="flex items-center space-x-1.5 py-2 px-3 text-xs font-bold rounded-xl bg-purple-600 hover:bg-purple-500 text-white shadow-purple-600/20 transition-all"
                  >
      ...
      {selectedMarketForSettle && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm">
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl space-y-5">
            <div>
              <h3 className="font-bold text-lg text-slate-100">Settle Market Result</h3>
              <p className="text-xs text-slate-400">{selectedMarketForSettle.eventName}</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                {selectedMarketForSettle.selections.map((sel) => (
                  ...
                  >
                    <span>{sel.name}</span>
                    {selectedWinningRunner === sel.selectionId && <CheckCircle className="w-4 h-4 text-purple-400" />}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="flex space-x-3">
              <button
                ...
                className="flex-1 py-2.5 text-xs font-semibold rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 py-2.5 text-xs font-bold rounded-xl bg-purple-600 hover:bg-purple-500 text-white disabled:opacity-50"
              >
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm">
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl space-y-4">
            <h3 className="font-bold text-lg text-slate-100">Create New Sporting Event</h3>
            <form onSubmit={handleCreateSubmit} className="space-y-3">
              <div>
                <label className="text-xs font-semibold text-slate-300">Market ID (Unique)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-xs font-semibold text-slate-300">Sport</label>
                <select
                  value={newSport}
              ...
              <div className="space-y-2 pt-2 border-t border-slate-800">
                <label className="text-xs font-semibold text-slate-300">Runners / Selections</label>
                <input
                  type="text"
              ...
              <div className="flex space-x-3 pt-3">
                <button
                  ...
                  className="flex-1 py-2 text-xs font-semibold rounded-lg bg-slate-800 text-slate-300"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="flex-1 py-2 text-xs font-bold rounded-lg bg-blue-600 hover:bg-blue-500 text-white"
                >
                  Create Market
                </button>
              </div>
